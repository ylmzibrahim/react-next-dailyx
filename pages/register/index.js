...
import { postUserRegister } from "../../network/lib/user";
...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (user.user.username.length < 8 || user.user.username.length > 16) {
      setMessage({ message: t("register.username.error"), type: "error" });
      return;
    } else if (!regexEmail.test(user.user.email)) {
      setMessage({ message: t("register.email.error"), type: "error" });
      return;
    } else if (user.user.password !== user.user.password_confirmation) {
      setMessage({ message: t("register.password.mismatch"), type: "error" });
      return;
    } else if (!regexPassword.test(user.user.password)) {
      setMessage({ message: t("register.password.error"), type: "error" });
      return;
    }

    postUserRegister(user)
      .then((response) => {
        setMessage({ message: `${t("register.success")}`, type: "success" });
      ...
      .catch((error) => {
        if (error.response.data.errors.username !== undefined)
          setMessage({
            message: t("register.username.exists"),
            type: "error",
          });
        else if (error.response.data.errors.email !== undefined)
          setMessage({
            message: t("register.email.exists"),
            type: "error",
          });
        else setMessage({ message: t("login.error"), type: "error" });
        console.error("postUserRegister => error: ", error);
      });
  };

  return (
    <div className="flex-col items-center justify-center mt-10 space-y-3">
    <Head>
      ...
      <div className={styles.input_container + " dark:bg-sky-800 dark:border-sky-800"}>
        <label htmlFor="register_username" className={styles.input_label + " dark:text-slate-500"}>
          {t("username.title")}
        </label>
        <input
      ...
      <div className={styles.input_container + " dark:bg-sky-800 dark:border-sky-800"}>
        <label
          ...
          className={styles.input_label + " dark:text-slate-500"}
        >
          {t("password_confirmation.title")}
        </label>
        <input
          ...
          onChange={(e) =>
            setUser({
              ...user,
              user: { ...user.user, password_confirmation: e.target.value },
            })
          }
          required
        />
